...
class Game:
    def __init__(self):
        ...
        for cur in self.stocks_by_currency.keys():
            ...
            for item in data_list[:20]:
                if cur == "원":
                    stocks.append(Stock(item["name"], item["price"], cur, max_loss_mult=0.9, max_gain_mult=1.05, bias=0.8))
                    #max_loss_mult=0.9, max_gain_mult=1.05, bias=0.8
                elif cur == "코인":
                    stocks.append(Stock(item["name"], item["price"], cur, max_loss_mult=0.001, max_gain_mult=100, bias=0.01))
                    #max_loss_mult=0.001, max_gain_mult=100, bias=0.01
                elif cur == "금":
                    stocks.append(Stock(item["name"], item["price"], cur, max_loss_mult=0.5, max_gain_mult=1.2, bias=0.85))
                    #max_loss_mult=0.6, max_gain_mult=1.2, bias=0.95
                elif cur == "스탁":
                    stocks.append(Stock(item["name"], item["price"], cur, max_loss_mult=0.4, max_gain_mult=1.6, bias=0.6))
                    #max_loss_mult=0.4, max_gain_mult=1.4, bias=0.6
            self.stocks_by_currency[cur] = stocks

        # 초기 차트 데이터 생성을 위해 몇 번 업데이트 실행 (10)
        for _ in range(10): #밸런싱 조절을 위해 임시로30
    ...
    def draw_shop_modal(self):
        ...
        dummy_text = self.font_md.render("ESC눌러서 나가세요", True, UI.COLOR_BLACK)
